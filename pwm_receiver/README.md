...
WDR v1 has no direct profile-readback command. Before replacing the original 100-frame profile, we reconstructed its commanded widths through finite Wi-Fi replay and `STATUS` sampling; see [the capture record](../scripts/README.md). Its metadata and exact rate remain unknown. We then used Athena to upload a 200-frame flight-log profile over Wi-Fi and ran one finite cycle with the receiver attached and no servos. The receiver recorded four output channels at 19,999–20,000 µs periods and pulse widths tracking the uploaded profile; [full evidence](../captures/real_wifi_flight_20260926.json) is saved. The bench firmware was not changed. The bench now holds the 200-frame profile, stopped at idle with `cycles=25 run_s=34 active_s=33,33,33,16`.
