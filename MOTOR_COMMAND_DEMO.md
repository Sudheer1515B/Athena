...
2. Start the ordinary backend if needed: `.venv/bin/python -m uvicorn backend.app:app --host 0.0.0.0 --port 8080`. Open <http://127.0.0.1:8080> on this Mac, or `http://<Mac Wi-Fi IPv4>:8080` on another device on the same Wi-Fi, and refresh after installing a new release build. The Mac's address on 26 September was `10.178.45.149` (it can change). This shared dashboard includes the controller buttons; use a trusted demo network. In Settings connect Wi-Fi to **10.178.45.105**, port **3333**. Connection alone does not drive outputs.
