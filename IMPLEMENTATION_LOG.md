...
# Entry 033 — 2026-09-26 — Four-motor extracted-command path

Scope/authorization: user connected a drone to all four bench outputs and requested sending extracted commands first, postponing monitoring integration. User reconfirmed removed propellers, secured assembly, attended motor-power cutoff, 1000-us motor stop and 2000-us full throttle. Existing manual nc SET success establishes protocol acceptance, not physical measurement.

Decision: native WDR upload/STOP/boot/finite completion emits 1500 us. Added a separate host-timed SET replay mode rather than using native servo replay for powered ESC testing. No bench or receiver firmware changes. Existing native endurance path and remote rollback branch are preserved.

Implementation: `backend/motor_replay.py` sequences four SETs per frame using the persisted profile's frame rate, finite passes and full-profile validation (four mapped outputs, checksum, 1000–approved ceiling, ceiling at most 2000). Default ceiling 1100. Initial/terminal/cancel requests low on all outputs and reads STATUS. Late scheduling beyond max(100 ms, two frame periods) aborts; lost ACK is not retried/restarted and low is unconfirmed when disconnected. Backend owns the stream and serializes all competing controller operations; native Stop is blocked during active motor sending. Added /motors/start, /motors/cancel and /motors/idle, progress in snapshots, separate history audit events with no invented native cycle/hour totals. Graceful backend shutdown requests cancel/low.

Flutter: Profile includes Motor commands with per-channel actual ranges, motor GPIO map, approved ceiling, finite passes, review confirmation, send/cancel/all-low controls and readback/error status. Native upload on physical benches explains 1500-us behavior and requires motor-power-off acknowledgement. Native Stop is labeled 1500 us. Added editable persisted channel min/max limits. Compiler version 2 permits mapped low-throttle ranges excluding 1500 while requiring unmapped fixed-1500 outputs to fit their limits; values are rejected, never clipped. Profile changes invalidate the motor review confirmation.

Validation: 40 backend unittest tests passed (including seven new motor tests); 21 Flutter tests passed (including two new motor UI tests); Flutter analyze clean; release web build succeeded. Initial Flutter run caught missing Material around CheckboxListTile; fixed and reran successfully. A temporary changed native card title broke the supplied structure test; restored the original four-stage title. Existing unrelated test SQLite ResourceWarning remains. Supplied simulator API integration passed exact 50-Hz frame values, finite completion/1000 readback, cancellation/1000 readback, rejected ceiling/missing safety confirmation, blocked competing native Stop/duplicate replay, dropped SET acknowledgement/unknown low, unchanged native counters and no STOP/START/LOAD/F/COMMIT/CLEAR/RESUME. Evidence: `/private/tmp/athena-motor-replay-verification.json` generated by `.venv/bin/python -u scripts/verify_motor_replay.py`, using isolated temporary storage and the official simulator only.

Runtime: replaced the identified older local backend PID 75643 with tested backend PID 76053 (tool session 96362), serving release UI at http://127.0.0.1:8080. Read-only reconnection via `scripts/motor_preflight.py` confirmed 10.178.45.105:3333, WDR_REFERENCE/proto1/ch4/maxframes8000, STOPPED, stored frames1200, observed us800,800,800,800. These widths were observed, not issued by this work. Current stored Athena 60-s/50-Hz profile `17627305-cc4e-42ae-a1ab-8f06cd2424db` maps C1–C4 to OUT0–OUT3 and peaks at1718 us, so default motor ceiling1100 rejects it. No real throttle profile, STOP, upload or counter reset was executed in this implementation step. Connection only sent read-only queries plus TIME sync.

Limitations: host SET frames are sequential and network-timed, not deterministic or atomic. This mode leaves native bench state STOPPED and does not accrue bench-native endurance cycles/hours. Lost Wi-Fi, process/host power loss, external command clients and bench reset can defeat the return-low request; physical motor-power cutoff remains necessary, and fixed firmware boot still emits1500. Receiver/drone monitoring integration is deferred as requested. Exact ESC manual and actual loaded timing/motor behavior remain unverified. See `MOTOR_COMMAND_DEMO.md` for execution and APIs.
# Entry 034 — 2026-09-26 — Motor monitoring and persistent frame recovery

Scope: user requested monitoring integration, clarified no receiver is connected, then requested motors stop on ESP power loss, automatic Wi-Fi reconnection and engineer-approved continuation where playback left off. User asserts motors stop when ESP power is cut. Treat this as supplied hardware behavior; no independent signal or mechanical telemetry is present. Fixed WDR firmware's 1500-us boot output remains unchanged and cannot be suppressed by a disconnected host.

Monitoring: added backend `motor_monitoring` snapshot with fresh bench STATUS widths, GPIO mapping, readback age/current all-four-low state, separate optional receiver electrical evidence and explicit null RPM/current/temperature. Dashboard Drone motor monitoring shows command graph, per-output commands, progress/pass/profile-rate/elapsed/lateness, profile identity, timestamps, historical low readback and motor cancel/low controls. Wi-Fi/backend staleness hides widths/graph; absent receiver remains UNKNOWN, never inferred from commands. Historical successful 1000 readback does not establish current output state. SET replay still does not accrue bench-native hours/cycles.

Recovery: `backend/motor_journal.py` atomically replaces an fsynced JSON checkpoint (including directory fsync) at `var/motor-checkpoint.json`. Immutable profile hash/ID, endpoint/capabilities, ceiling/rate/pass target, next complete-frame cursor and in-flight intent are persisted. Intent precedes each four-SET frame; acknowledgement cursor follows all four replies. On fault or shutdown/backend restart, the recovery remains awaiting explicit review. Backend startup can restore the pinned observation connection from the journal. Monitor reconnects automatically and, only for a matching pending motor recovery, requests all four1000 and checks STATUS. This request occurs after network rejoin; it does not eliminate the earlier1500 boot transient. No throttle frames/native Start/upload are automatic. Motor idle preserves the outstanding cursor. Active sending and pending motor recovery block native servo output controls.

Approval: cross-tab recovery banner reports saved pass/frame and partial-frame uncertainty. Review requires fresh STOPPED/all-four1000 bench readback, original endpoint/capabilities/profile identity/checksum/limits/cursor bounds, same physical bench/wiring review, hardware failsafe/boot protection, props/secured assembly/cutoff and uncertainty acknowledgement. Approval is consumed before commands; duplicate requests are denied. Host playback sends only remaining immutable frames. An interrupted partial frame repeats in full. Last-frame dwell/physical motor position are unknown, so this is command-frame continuation, not exact physical continuity. Native servo recovery remains whole-cycle/frame0 because WDR has no seek command.

Validation: 48 backend tests pass, including monitoring provenance/staleness/NO SIGNAL vs UNKNOWN and durable partial-frame/restart/remaining-frame replay/checkpoint-disk-failure tests. 26 Flutter tests pass, including command-vs-physical evidence, stale graph removal, cancel availability and approval-gated saved-frame resume. Analyze clean, release web build successful. Supplied simulator integration passes exact50Hz frames with durable checkpoints, cancellation-low, partial lost SET ACK, reboot, automatic reconnect/low without throttle, backend restart with preserved cursor, missing approval rejection, exact remaining-command continuation, duplicate approval rejection, frozen native controls, unknown physical telemetry and unchanged native counters/no native motion/upload/reset commands. Evidence `/private/tmp/athena-motor-replay-verification.json`. Existing native servo regression also passes upload progress, browser-absent completion, lost Start/Stop ACK, six-second outage, interrupted upload and approval recovery; evidence `/private/tmp/athena-monitoring-native-regression/results.json`. An initial requested2-second regression duration was rejected by the harness (minimum6); rerun with6 passed. Preexisting test SQLite ResourceWarning remains.

Real/runtime evidence: read-only inspection of old backend showed engineer-triggered profile `2e5cbaa7-63b1-4845-b668-fd43b52e1db7` COMPLETED600/600 frames,60.212s elapsed,0.032s maximum scheduling delay and successful low readback (persisted profile60s at10Hz). This is host command evidence, not mechanical proof. Replaced backend only while replay inactive. Final PID77209/tool session54544 serves release at http://127.0.0.1:8080. Reconnected WDR_REFERENCE at10.178.45.105:3333 using status/time-sync only; confirmed STOPPED,1800 stored bench frames and1000,1000,1000,1000. Monitoring correctly showed command_fresh=true/current_all_low_reported=true and all physical telemetry null/UNKNOWN. No real profile, motor recovery, power-cut demo, firmware flash, native STOP/upload or counter clear was executed by automation in this step. Old pre-journal completed action remains in History; this upgrade creates durable frame checkpoints for new runs.

Rollback: `pre-motor-monitoring-20260926` pins prior tested motor-send commit5ec8aca; `pre-motor-replay-20260926` and original prototype rollback also remain. Instructions/API/hardware boundaries are updated in `MOTOR_COMMAND_DEMO.md`.
